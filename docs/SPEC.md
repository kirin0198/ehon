...
>   - 2026-05-06: タッチスワイプ Phase 1 (developer / UC-006 にスワイプ操作追加 / 受入基準追記)
>   - 2026-05-05: ページめくりアニメ強化 Phase 2 (developer / UC-006 受入基準にアニメの立体感を追記 / CSS only / バンドル増 0 kB)
>   - 2026-05-05: ViewerA / ViewerB の RTL 化 (developer / UC-006 受入基準のスワイプ方向を反転、見開きレイアウト記述を右綴じ仕様に / キーボード ←/→ は OS 標準維持 / バンドル増 0 kB)
...
- 表紙 + 「よみはじめる」CTA、ページ送り（ボタン / キーボード ←/→ / タッチスワイプ）、Esc でビュアー閉じる
...
- **概要**: 本文ページを進めたり戻したりする
- **正常フロー**:
  1. ナビボタン（◀ まえ / ▶ つぎ, 56px 円形）で送る
  2. キーボード: OS 標準（← で前 / → で次）で送る
  3. タッチスワイプ（**右で次 / 左で前**, 右綴じ仕様）で送る
  4. 最終ページに到達したら次送りは無効化
- **受入基準**:
  - 全 3 操作系（ボタン / キーボード / タッチスワイプ）が機能する
  - 進捗バー（4px）が現在位置を反映する（Should）
  - `prefers-reduced-motion: reduce` のとき遷移アニメは停止 / 短縮
  - 50px 以上のスワイプで前/次ページに遷移する（**右スワイプ = 次ページ / 左スワイプ = 前ページ**）
  - キーボードは OS 標準: → で次ページ / ← で前ページ（スワイプ方向とは独立して維持）
  - 500ms のフリップロック中はスワイプも無視される
  - 垂直方向のスワイプは無視される（縦スクロールを阻害しない）
  - スワイプはタッチデバイスで機能（`trackTouch=true`）。マウスでは反応しない（`trackMouse=false`）
  - ViewerA 見開き: **右ページが現在ページの絵 / 左ページが現在ページの文**（右綴じ書籍仕様 / DOM 順序も right → left でスクリーンリーダーの reading order と一致）
  - ViewerA 単独ページ（表紙・奇数番目終わり最終ページ）は右側に表示
  - ViewerB のスワイプ方向も ViewerA に同期（右=次 / 左=前）
  - ページ送り時にめくり中の影 + perspective による立体感が視覚的に確認できる（`prefers-reduced-motion: reduce` 時は除外）
  - アニメ強化はバンドル増 0 kB（CSS only / ライブラリ追加なし）
...
- [ ] (2026-05-06 追加) タブレット / スマホでスワイプによるページ送りが機能する (**右→次 / 左→前**, 右綴じ仕様)
- [ ] (2026-05-05 追加) ページめくり時に紙の厚み・立体感が CSS only のアニメで表現される（`prefers-reduced-motion` 時は除外）
- [ ] (2026-05-05 追加) ViewerA / ViewerB が右綴じ書籍仕様で動作する（スワイプ右=次, 左=前 / キーボードは OS 標準維持）
